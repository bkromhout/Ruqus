...
public class LessThan extends RUQTransformer {
    ...
    public <T extends RealmObject> RealmQuery<T> transform(RealmQuery<T> realmQuery, Condition condition) {
        ...
        if (Names.DATE_CANON_NAME.equals(fieldTypeName)) return realmQuery.lessThan(field, (Date) args[0]);
        else if (Names.DOUBLE_CANON_NAME.equals(fieldTypeName)) return realmQuery.lessThan(field, (Double) args[0]);
        else if (Names.FLOAT_CANON_NAME.equals(fieldTypeName)) return realmQuery.lessThan(field, (Float) args[0]);
        else if (Names.INTEGER_CANON_NAME.equals(fieldTypeName)) return realmQuery.lessThan(field, (Integer) args[0]);
        else if (Names.LONG_CANON_NAME.equals(fieldTypeName)) return realmQuery.lessThan(field, (Long) args[0]);
